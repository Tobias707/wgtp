...
**Vault:** `D:\Obsidian\second brain`
**Project Note:** `D:\Obsidian\second brain\02 Projekte\What Game to Play.md`

### Session Start — always do this automatically
Read the project note at the path above. Use it as source of truth for current status, open problems, and next steps.

### Session End / "wrap it up" — always do this automatically
Update the project note:
1. Check off completed items in `### ✅ Erledigt`
2. Update `### ⚠️ Bekannte Probleme` — add new, remove fixed
3. Update `### 📋 Nächste Schritte` checkboxes
4. Append entry to `## Arbeitslog` with today's date and bullet-point summary of what was done

Do this without being asked whenever the session ends or "wrap it up" is said.

---

## custom commands

When i say "wrap it up" sum the context of the current chat so i can take it to a different chat — AND update the Obsidian project note as described above.
...
- [x] Test suite: 100 diverse profiles (genres, budgets, platforms, difficulty, popularity, edge cases) — **100% pass rate on Railway API** (2026-05-04)
...
└── tests/
    ├── test_profiles.json       ← 100 test profiles (genres, budgets, platforms, etc.)
    ├── run_tests.py             ← test executor (calls /api/recommend for each profile)
    └── test_results.json        ← output (generated by run_tests.py)
```
...
**Problem identified via test analysis:**

Users selecting specific genres (Horror, Shooter) were getting wrong games:
- Horror profile: Minecraft Story Mode, The Darkest Tales (NOT horror)
- Shooter profile: Path of Exile (NOT shooter)
- Simulation profile: Ultimate Epic Battle Simulator (NOT cozy/farming sim)

Root cause: Soft penalty (-80) insufficient. Games with ANY matching tag pass through, even if it's the wrong context.

**Solution: Hybrid Genre Filtering (Core + Blend Genres)**

Introduced two-tier filtering:

**CORE GENRES (hard filter - user MUST get exact match):**
- `horror` → game MUST have "horror", "survival horror", or "psychological horror" tag
- `shooter` → game MUST have FPS/shooter-specific tag ("fps", "first-person shooter", etc)
- `roguelike` → game MUST have "rogue-like", "roguelite", "action roguelike" tag
- `puzzle` → game MUST have "puzzle" or puzzle-specific tags
- `simulation` → game MUST have "farming sim", "life sim", "tycoon", "management" (filters "battle simulator")

**BLEND GENRES (soft penalty -80 if no match):**
- `action`, `adventure`, `rpg`, `platformer`, `strategy`, `sandbox`, `sports`

**Fixes applied (commit TBD):**

1. **Core genre hard filtering** — Moved Horror, Shooter, Roguelike, Puzzle to strict tag requirements (hard filter, not soft penalty)
2. **Simulation tag refinement** — Filtered simulation to exclude generic "Simulation" tag without proper subtype (farming sim, tycoon, etc)
3. **Blend genres soft penalty** — Action/Adventure/RPG/etc still use -80 penalty for flexibility

**Test Results (2026-05-07):**
- Horror + Adventure: ✅ Stories Untold, Among the Sleep, Urban Myth Dissolution (NO Minecraft)
- Shooter: ✅ Counter-Strike 2, GUN, SharpShooter3D (NO Path of Exile)
- Sandbox + Simulation: ✅ Ranch Simulator, RCT Deluxe, Cooking Simulator (NO Ultimate Epic Battle Simulator)

---

## Testing

**Test Suite (2026-05-04):**
- 100 profiles in `tests/test_profiles.json` covering:
  - Single/multi-genre combinations
  - All difficulty levels (chill, easy, medium, hard)
  - All budget tiers (free, <5, <10, <30, any)
  - All platforms (PC, Switch, PS, Xbox, mobile, Steam Deck)
  - Loved game anchoring (Stardew, Hades, Portal 2, etc.)
  - Popularity extremes (hidden gems to blockbusters)
  - Co-op player counts (solo, 2, 3-4, 5+)
  - Edge cases (impossible combos, empty genres, stress tests)

**Run tests:** 
```powershell
cd d:\ProjektWGTP
python tests/run_tests.py  # or: "C:\Users\tobia\AppData\Local\Programs\Python\Python311\python.exe" tests/run_tests.py
```

**Result (2026-05-04):** 100/100 profiles passed against Railway production API. All returned 10 game recommendations. Zero crashes or errors. ✅

---

## Known Issues

**Python PATH:** Windows app alias blocks `python` command. Fix: Add to system PATH via Settings > Environment Variables:
- `C:\Users\tobia\AppData\Local\Programs\Python\Python311`
- `C:\Users\tobia\AppData\Local\Programs\Python\Python311\Scripts`

No reinstall needed. Once added, `python` will work globally in any terminal.
...
*Last updated: 2026-05-04 — comprehensive test suite complete, 100/100 profiles passing*
